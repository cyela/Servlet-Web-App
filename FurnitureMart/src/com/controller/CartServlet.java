		...
		if (action.equals("cart_item")) {
			...
			try {
				...
				if (bufList != null) {
					for (int i = 0; i < bufList.size(); i++) {
						if (bufList.get(i).getEmailId().equals(user.getEmail())
								&& bufList.get(i).getProductId().equals(productId)) {
							check++;
						}
					}
					if (check != 0) {
			...
			try {
				...
				if (bufList != null) {
					float sum = 0;
					for (int i = 0; i < bufList.size(); i++) {
						sum = sum + (Float.valueOf(bufList.get(i).getPrice())
								* Float.valueOf(bufList.get(i).getQuantity()));
					}
			...
			try {
				...
				for (int i = 0; i < bufList.size(); i++) {
					sum = sum
							+ (Float.valueOf(bufList.get(i).getPrice()) * Float.valueOf(bufList.get(i).getQuantity()));
				}
			...
			for (int i = 0; i < bufList.size(); i++) {
				...
				order.setTotalCost(Float.toString(
						Float.parseFloat(bufList.get(i).getPrice()) * Float.parseFloat(bufList.get(i).getQuantity())));
				// --cart details
